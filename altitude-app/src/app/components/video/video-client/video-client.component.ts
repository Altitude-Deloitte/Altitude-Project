...
export class VideoClientComponent {
  ...
  processChatResponse(generationData: any) {
    console.log('Processing chat response in video client:', generationData);

    // Update component data based on chat response
    if (generationData.image_url || generationData.video_url) {
      const url = generationData.video_url || generationData.image_url;
      this.imageUrl = url;
