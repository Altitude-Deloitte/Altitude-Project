...
export class ProductDescComponent {
  ...
  navigateToForm(): void {
    this.route.navigateByUrl('generate-product-desc');
  }
  ...
  /*onFileSelected(event: any): void {
    ...
    const {wordLimit} = formValues ;
    const {lang} = formValues ;
      ...
      Papa.parse(csv, {
        ...
        complete: (result) => {
          this.csvData = result.data.map((row: any) => ({
            imageUrl: row.ImageUrl,
            attributes: row.Product +row.Product_Specification + row.Materials +row.Keywords+ "Tone: "+ Type +", Wordlimit: "+wordLimit+", Language: "+lang+", Vocabulary: "+language,
          }));
          console.log('Parsed CSV Data:', this.csvData[0].attributes);
          console.log('Parsed CSV Data:', this.csvData[0].imageUrl);
            // Call service for each row
            this.csvData.forEach((data) => {
              this.callGenerateDescriptionApi(data);
            });
